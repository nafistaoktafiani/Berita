...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
            ...
            $query = "UPDATE teknologi SET judul='$judul', deskripsi='$deskripsi', foto='$foto' WHERE id='$id'";
        }
    } else {
        // Update tanpa mengubah foto
        $query = "UPDATE teknologi SET judul='$judul', deskripsi='$deskripsi' WHERE id='$id'";
    }

    $query_run = mysqli_query($conn, $query);
    if ($query_run) {
        echo "<script>alert('Data teknologi berhasil diperbarui');</script>";
